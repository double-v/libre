...
async function computeAnalytics(totalUsers: number): Promise<AnalyticsStats> {
  ...
  const [
    ...
    getDb().$queryRaw<Array<{ value: string; count: bigint }>>`
      ...
      FROM (SELECT unnest(orientation) as value FROM "profiles") t
      GROUP BY value
      ORDER BY count DESC
    `,
    getDb().$queryRaw<Array<{ value: string; count: bigint }>>`
      SELECT value, COUNT(*) as count
      FROM (SELECT unnest("relationshipType") as value FROM "profiles") t
      GROUP BY value
      ORDER BY count DESC
    `,
    getDb().$queryRaw<Array<{ value: string; count: bigint }>>`
      SELECT value, COUNT(*) as count
      FROM (SELECT unnest(interests) as value FROM "profiles") t
      GROUP BY value
      ORDER BY count DESC
    `,
    getDb().$queryRaw<Array<{ value: string; count: bigint }>>`
      SELECT value, COUNT(*) as count
      FROM (SELECT unnest(practices) as value FROM "profiles") t
      GROUP BY value
      ORDER BY count DESC
    `,
