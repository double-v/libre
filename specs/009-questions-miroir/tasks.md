...
- [ ] T026 [P] [US4] Tests `src/components/__tests__/ThisOrThat.test.tsx` : une paire à la fois, toucher → PUT puis paire suivante sans réponse, « Passer » n'enregistre rien, sortie libre, aucun compteur
- [ ] T027 [US4] `src/components/ThisOrThat.tsx` (après T003), lancé depuis la section du profil ; sur la fiche, les paires visibles s'affichent en ligne compacte (`AnswerBlock`)
...
- [ ] T021 [P] Bloc `answers` de `GET /api/admin/stats` (R8 : part des profils avec ≥ 1 réponse ; part n'ayant répondu que par des choix, SC-006 ; part des conversations dont les deux membres partagent une question, vs hasard) + test
- [ ] T022 [P] `CLAUDE.md` (section Sécurité : règle miroir des réponses, garde `answers-never-leak`, contact partagé `contact.ts`), `DESIGN.md` (motif pastilles + précision, « Ceci ou cela »), CGU : clause « ne pas promouvoir ni proposer de produits illicites dans les réponses »
