...
- **`TopNav`** (DS, `src/components/ui/TopNav.tsx`) : **en-tête unifié**, présent
  ...
  (guest) ou `/discover` (connecté) ; à droite, le **`ThemeMenu` toujours**, puis
  les actions selon l'état de session :
...
- **`ThemeMenu`** (DS, `src/components/ui/ThemeMenu.tsx`) : cf. § dédié plus bas.
...
| `ThemeMenu` | closed, open, focus-trap | popover (desktop), bottom-sheet (mobile) | `ThemeToggle` flottant, `LobbyThemeSwitcher`, radios d'apparence |
...
**Un seul bouton** dans le `TopNav`, partout — c'est *le* point d'entrée du
theming (mode **et** thème réunis). Remplace les trois sélecteurs historiques
(`ThemeToggle` flottant, `LobbyThemeSwitcher`, radios `AppearanceSettings`).
