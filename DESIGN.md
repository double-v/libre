...
- **Sélectionnable par l'utilisateur** via `ThemeMenu` (popover mode×thème) **en admin** (zone power, sans nav app ni page Paramètres), et **miroir dans les Paramètres** (`AppearanceSettings`) pour l'app connectée. Le nav de l'app (`SiteNav`) ne porte qu'un `ThemeToggle` (axe **Mode** seul) ; **la landing ET les pages `(auth)` n'exposent aucun sélecteur** (contextes invités → thème par défaut du site). *(Réconcilié par le shell #273 : #281, 2026-07-13.)*
...
(`AppearanceSettings`). Les **contextes invités — landing ET `(auth)`** —
n'exposent **aucun** sélecteur (thème = défaut du site). `ThemeMenu` (popover
complet) ne subsiste **qu'en admin** (zone power, sans nav app ni page
Paramètres). Tranché #281 (2026-07-13).
...
Porté par l'**admin uniquement** — zone power sans nav app ni page Paramètres, où
l'on veut exposer le choix de thème sur place (#281, 2026-07-13). Les **contextes
invités — landing ET `(auth)`** — n'exposent **aucun** sélecteur (thème par défaut
du site) ; l'app connectée porte un `ThemeToggle` (mode seul) dans `SiteNav`, le
choix du thème passe par les Paramètres (`AppearanceSettings`). Consolide les
sélecteurs ad hoc historiques (`LobbyThemeSwitcher`, radios en double).
